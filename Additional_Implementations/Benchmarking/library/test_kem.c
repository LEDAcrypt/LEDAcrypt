...
if(machine_readable == 0) {
   fprintf(stderr,"\n --------------------------------------------------------");
   fprintf(stderr,
           "\n  %1sLEDAcrypt-KEM (IND-CCA2 key encapsulation mechanism \n %1s with long-term public/private keypair) %1s ", " ", " ", " ");
   fprintf(stderr,"\n --------------------------------------------------------");
   fprintf(stderr,"\n  %54s "," ");
   ...
   fprintf(stderr,"\n SIZE: %d & %s & %d & %5lu & %5lu & %5lu & %5lu & %5lu \\\\\n",
         ...
         N0,
          ...
          (long unsigned int) ((CRYPTO_CIPHERTEXTBYTES)),
          (long unsigned int) ((CRYPTO_BYTES)),
          (long unsigned int) ((CRYPTO_CIPHERTEXTBYTES))); /* sent data */
}
...
void print_cycles_metrics(char tag[],
 ...
 fprintf(stderr,
         ...
         (welford_mean(encrypt_stats)+
          welford_mean(decrypt_stats)+
          (CRYPTO_CIPHERTEXTBYTES)*METRIC_ALPHA_FACTOR)/1E+3);
}
...
{
   if (NumTests < 2) {
      fprintf(stderr, "\n\n Less than two tests do not allow to compute std. dev.");
   }
